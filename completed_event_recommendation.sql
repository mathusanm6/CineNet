...
FROM
    ...
    JOIN Events ea ON a.event_id = ea.id
        AND ea.status = 'Completed'
    JOIN Events eb ON b.event_id = eb.id
...
FROM
    ...
    JOIN Events e ON s.event_id2 = e.id
        AND e.status = 'Completed' -- Only include completed events
WHERE
