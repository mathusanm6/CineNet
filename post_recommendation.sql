...
BEGIN
    RETURN CASE
        WHEN reaction = '😄' THEN 5 -- Excellent
        WHEN reaction = '🙂' THEN 4 -- Good
        WHEN reaction = '😐' THEN 3 -- Average
        WHEN reaction = '🙁' THEN 2 -- Poor
        WHEN reaction = '😩' THEN 1 -- Terrible
        ELSE 0 -- In case of unrecognized emoji
    END;
END;
$$ LANGUAGE plpgsql;
...
INSERT INTO 
    PostSimilarities (post_id1, post_id2, similarity)
SELECT 
    a.post_id AS post_id1, 
    b.post_id AS post_id2,
    (
        SUM(reaction_to_weight(a.emoji) * reaction_to_weight(b.emoji)) / 
        (SQRT(SUM(POWER(reaction_to_weight(a.emoji), 2))) * SQRT(SUM(POWER(reaction_to_weight(b.emoji), 2))))
    ) AS similarity
FROM 
    Reactions a
...
GROUP BY 
    ...
    b.post_id
HAVING 
    COUNT(a.user_id) > 1;  -- Ensure enough overlap for meaningful similarity
...
FROM 
    ...
    LEFT JOIN Reactions rm ON u.id = rm.user_id AND s.post_id2 = rm.post_id
WHERE 
