...
INSERT INTO 
    MovieSimilarities (movie_id1, movie_id2, similarity)
SELECT 
...
FROM 
    UserMovieRatings a
    JOIN UserMovieRatings b ON a.user_id = b.user_id 
...
FROM 
    UserMovieRatings r
    JOIN MovieSimilarities s ON r.movie_id = s.movie_id1
    JOIN Users u ON u.id = r.user_id
    LEFT JOIN UserMovieRatings rm ON u.id = rm.user_id 
    AND s.movie_id2 = rm.movie_id
WHERE 
