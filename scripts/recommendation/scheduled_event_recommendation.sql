...
END IF;
    IF NOT EXISTS(
        ...
        FROM
            information_schema.tables
        WHERE
            table_name = 'movierecommendation') THEN
...
SELECT
    ...
    COUNT(*)::numeric /(
        ...
        FROM
            Participation
        WHERE
            user_id = p1.user_id) AS similarity
FROM
    Participation p1
    JOIN Participation p2 ON p1.event_id = p2.event_id
...
FROM
    ...
    LEFT JOIN Participation p2 ON us.user_id1 = p2.user_id
        AND p.event_id = p2.event_id
    LEFT JOIN Screenings s ON p.event_id = s.event_id
    LEFT JOIN MovieRecommendation mr ON us.user_id1 = mr.user_id
